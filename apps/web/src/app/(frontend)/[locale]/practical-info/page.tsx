...
export async function generateMetadata({ params }: Props) {
  ...
  const page = await getPageBySlug('practical-info', locale as any)
  return {
    title: page?.hero?.headline ?? (locale === 'de' ? 'Praktische Infos für Marienbad' : 'Practical Info for Marienbad'),
    description: 'Everything you need to know before visiting Marienbad – getting here, weather, maps, events, and travel tips.',
  }
}

export default async function PracticalInfoPage({ params }: Props) {
  const { locale } = await params
  ...
  return (
    <>
      <HeroSection
        title={page?.hero?.headline ?? t('nav.practicalInfo')}
        subtitle={page?.hero?.subheadline ?? 'Everything you need to plan your visit to Marienbad'}
